/**
 * NemoPromptTools - Centralized Logging Utility
...
class Logger {
    constructor(prefix = 'NemoPromptTools', level = CONSTANTS.LOG_LEVELS.INFO) {
        this.prefix = prefix;
        this.level = level;
...
if (isDebugEnabled()) {
    ...
    console.log('[NemoPromptTools] Debug mode enabled');
}

// Export utility to toggle debug mode at runtime
// Usage: window.NemoPromptToolsLogger.enableDebug() or .disableDebug()
if (typeof window !== 'undefined') {
    window.NemoPromptToolsLogger = {
        enableDebug: () => {
            logger.setDebugMode(true);
            try { localStorage.setItem('nemoPresetExtDebug', 'true'); } catch (e) {}
            console.log('[NemoPromptTools] Debug mode enabled (persisted to localStorage)');
        },
        disableDebug: () => {
            logger.setDebugMode(false);
            try { localStorage.removeItem('nemoPresetExtDebug'); } catch (e) {}
            console.log('[NemoPromptTools] Debug mode disabled');
        },
        getLogger: () => logger
    };
}
